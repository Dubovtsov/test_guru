class BadgeRule < ApplicationRecord
  ...
  def self.reward(test_passage, current_user)
    ...
    @user_test_passages.each do |test_passage|
      @successful_tests.push(test_passage) if test_passage.successfully?
    end

    if @successful_tests.size == 1
    ...
    elsif @successful_tests == @all_tests_in_the_category && @user_test_passages.count == @successful_tests
